...
- [ ] M2 (state + expand toggle) — add `expanded: Option<u32>` to `ReviewState`; add `ReviewActions.toggle_expand: Option<u32>`; the 📷 button toggles it; `render_review_window` dispatches it (sets/clears `expanded`, loads that row's texture into `review.preview`).
